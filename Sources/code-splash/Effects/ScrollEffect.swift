...
class ScrollEffect: VisualEffect {
    ...
    func execute(text: String, in window: NSWindow) {
        ...
        let vy: CGFloat = (scrollUp ? 1 : -1) * max(3000, (textField.frame.height + contentView.frame.height) / 1.0)
        ...
        animative = Animative(textField, bounds: contentView.bounds, vy: vy)
    ...
    private struct Animative {
        ...
        init(
            _ view: NSTextField, bounds: CGRect, vy: CGFloat
        ) {
            self.view = view
            self.bounds = bounds
            self.vy = vy
